...
import { GradeClassSelector } from './GradeClassSelector';
import { PreviewTable } from './PreviewTable';
import { ImportResult as ImportResultComponent } from './ImportResult';
import {
  useRecordsImportPreview,
  useRecordsImportExecute,
...
export function RecordsImportWizard() {
  ...
  const previewMutation = useRecordsImportPreview();
  const executeMutation = useRecordsImportExecute();
  const cancelMutation = useCancelImportPreview();

  const canProceedToUpload = selectedSchool && grade !== null && className.trim() !== '';
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    if (!selectedSchool || grade === null || !className.trim()) return;
  ...
  }, [selectedSchool, grade, className, previewMutation]);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        {/* Step 1: Select School + Grade + Class */}
        {step === 'select' && (
          <div>
            <div className="mb-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-2">選擇匯入範圍</h2>
              <p className="text-gray-600">請選擇要匯入運動記錄的學校、年級和班級</p>
            </div>
          ...
          <div>
            <div className="mb-6">
              ...
              <div className="text-gray-600 space-y-1">
                ...
                <p>
                  年級班級：<span className="font-medium">{grade} 年 {className} 班</span>
                </p>
              </div>
            </div>
          ...
          <div>
            <div className="mb-6">
              ...
              <div className="text-gray-600 space-y-1">
                ...
                <p>
                  目標：<span className="font-medium">{selectedSchool?.name} {grade}年{className}班</span>
                </p>
              </div>
            </div>

            <PreviewTable rows={preview.rows} type="records" />
              ...
              <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-yellow-800">
                    包含 {previewStats.warning} 筆有警告的資料（數值可能超出正常範圍）
                  </span>
                </label>
              </div>
            )}
...
export default RecordsImportWizard;
